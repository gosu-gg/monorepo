...
contract Gosu is Ownable {
    ...
    function register(string calldata playerTag) public {
        require(
            ...
            "player tag already assigned to an address"
        );
        require(
            keccak256(bytes(addressToTag[msg.sender])) == keccak256(bytes("")),
            "address already assigned to a player tag"
        );
        require(
            keccak256(bytes(playerTag)) != keccak256(bytes("")),
            "can't have a empty player tag"
        );
    ...
    function createGame() public payable {
        ...
        if (
            ...
            games[gameIndex].state == GameState.RUNNING &&
    ...
    function joinGame(uint256 gameId) public payable {
        ...
        //CHECK IF LAST GAME IS A DRAW BY LIMIT OF TIME
        if (
            curGameP2 < games.length &&
            games[curGameP2].state == GameState.RUNNING &&
        ...
        require(
            ...
            "Player doesn't have active game"
        );
